...
use NZS\MS\Report;
use NZS\MS\Command\Subcommand\tocaosubcommand;
use NZS\MS\Command\Subcommand\xemtocaosubcommand;
...
class tocaoUI
{
    ...
    public function tocaoForm($player)
    {
        ...
        $f->setContent("§l§b•§a Cấp Độ hiện tại: §c".$this->getPlugin()->seeLevel($name)."\n§lb•§a Exp: §c".$this->getPlugin()->seeExp($name)."§f/§a".$max."\n§l§b•§a Số Lần tố cáo: §c".$this->getPlugin()->seeNB($name));
        $f->addButton("§l§c• §aTố cáo §c•", 0);
        $f->addButton("§l§c• §eHủy §cTố Cáo •", 1);
        $f->addButton("§l§c• §aHướng Dẫn§c •", 2);
